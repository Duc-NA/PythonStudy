...
import { changeEmail } from '../core/_request'
...
const SignInMethod: React.FC = () => {
  ...
  const formik1 = useFormik<IUpdateEmail>({
    ...
    onSubmit: async (values) => {
      values.currentEmail = currentUser?.email;
      setLoading1(true)
      try {
        const {data: result} = await changeEmail(values);
        if(result.success){
  ...
  const formik2 = useFormik<IUpdatePassword>({
    ...
    onSubmit: (values) => {
      setLoading2(true)
      setTimeout((values:IUpdatePassword) => {
        ...
        <div className='card-body border-top p-9'>
          <div className='d-flex flex-wrap align-items-center'>
            <div id='kt_signin_email' className={' ' + (showEmailForm && 'd-none')}>
              ...
              <div className='fw-bold text-gray-600'>support@devbyspark.com</div>
            </div>
